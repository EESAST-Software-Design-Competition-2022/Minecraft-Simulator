...
{
    public class Text_of_pigman : MonoBehaviour
    {
        int line0;
        int linecount1 = 0;
        ...
        public void Text()
        {
            using (StreamReader sr = new StreamReader(Application.streamingAssetsPath + name))
            {
                ...
                if (((line = sr.ReadLine()) != null))
                {
                    ...
                    if (linecount1 > line0 && num == 0&&numerical.material>=50)
                    {
                        Instantiate(object1, new Vector2(0, 0), Quaternion.identity);//可选择付款
                        num++;
                    }
                    else if (linecount1 > line0 && num == 0)//没钱
                    {
